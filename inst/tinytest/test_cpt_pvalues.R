...
test_xytdata <- function() {
  ...
  # xyt <- cpttestdata
  set.seed(2025L)
  cpt_xyt <- change_point_test_pvalue_xyt(x = xyt[, "easting"],
                                   y = xyt[, "northing"],
                                   time = xyt[, "time"],
                                   q_max = 3, N = 100, tol = 0)
  ...
  # cat(deparse(cpt_xyt))
  results_orig <- structure(c(1, 0.67, NA, NA, 0.51, 0.41, 0.2, NA, 0.64, 0.94,  0.68, 0.91, 0.59, NA, 0.92, 0.17, 0.16, 0.12, NA, 0.25, 0.93,  0.44, 0.04, NA, 0.42, 0.07, 0.89, 0.33, NA, 0.19, 0.29, 0.77,  0.46, 0.54, 0.89, 0.2, NA, NA, 0.94, 0.33, NA, 0.26, 0.59, 0.42,  1, 0.61, 0.45, 0.94, NA, NA, 1, 0.34, NA, NA, 0.07, 0.08, 0.21,  NA, 0.86, 0.41, 0.56, 0.69, 0.59, NA, 0.17, 0.03, 0.04, 0.07,  NA, 0.29, 0.94, 0.15, 0.05, NA, 0.09, 0.2, 0.67, 0.08, NA, 0.09,  0.28, 0.97, 0.25, 0.43, 0.53, 0.67, NA, NA, 0.58, 0.23, NA, 0.16,  0.37, 0.51, 0.85, 0.92, 0.57, NA, NA, NA, 1, 0.12, NA, NA, 0.09,  0.13, 0.4, NA, 0.9, 0.43, 0.27, 0.6, 0.17, NA, 0.05, 0.02, 0.01,  0.27, NA, 0.29, 0.38, 0.07, 0.02, NA, 0.18, 0.13, 0.24, 0.05,  NA, 0.04, 0.65, 0.77, 0.39, 0.64, 0.84, 0.7, NA, NA, 0.52, 0.22,  NA, 0.17, 0.5, 0.49, 0.93, 0.89, NA, NA, NA, NA), dim = c(50L,  3L), class = c("change_point_test_pvalue", "matrix", "array"), dimnames = list(     NULL, c("q=1", "q=2", "q=3"))) 
...
test_input_formats <- function() {
  ...
  data_move2 <- mt_as_move2(data.frame(t=as.POSIXct(t), x = x, y = y, id = 1),
                            ...
                            crs = 32631)
  set.seed(2025L)
  result_move2 <- change_point_test_pvalue(data_move2, q_max = 2, N = 50, tol = 0)
  # FIXME: test
  ...
  data_sftrack <- as_sftrack(data.frame(t=as.POSIXct(t), x = x, y = y, id = 1),
                             ...
                             crs = 32632)
  set.seed(2025L)
  result_sftrack <- change_point_test_pvalue(data_move2, q_max = 2, N = 50, tol = 0)
  # FIXME: test
}
...
test_multiple_paths <- function() {
  ...
  set.seed(2025L)
  cpt_mpaths <- change_point_test_pvalue(data = paths_trackframe,
                                      q_max = 3, N = 10, tol = 0)
  
  expect_equal(dim(paths_trackframe)[1],  sum(sapply(cpt_mpaths, NROW)))
  track1 <- split_by_id(paths_trackframe)[[1]]
  set.seed(2025L)
  cpt_path1 <- change_point_test_pvalue(data = track1,
                                         q_max = 3, N = 10, tol = 0)
  ...
  set.seed(2025L)
  cpt_mpaths_sftrack <- change_point_test_pvalue(data = paths_sftrack,
                                         q_max = 3, N = 10, tol = 0)
