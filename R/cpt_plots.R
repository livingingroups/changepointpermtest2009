...
plot.trackframe <- function(
  ...
  if(any(names(args) %in% restricted)) {
    warning(sprintf("argument %s is restricted and therefore ignored",
...
#' @noRd
plotcpt <- function(data, alpha = 0.05, q = 4, N = 10000, tol = 0, clu = NULL, seed = NULL, ...) {
  UseMethod("plotcpt")
}
...
plot_n_cp_by_q <- function(data, id_col, ...) {
  ...
  default_options <- list(type = type_spline(n= 100),
                          grid = TRUE)
...
#' 
#' 
type_arrows <- function (x0, y0, x1, y1, length = 0.25, angle = 30, code = 2,
                         arrow_col = "black", arrow_lty = par("lty"), arrow_lwd = par("lwd")) 
{
  # assert_numeric(x0)
  data_arrows = function(datapoints, lwd, lty, col, ...) {
    ...
    return(list(type_info = list(ul_lty = ul_lty, ul_lwd = ul_lwd, 
                                 ul_col = ul_col)))
  }
  draw_arrows = function() {
    fun = function(ifacet, iby, data_facet, icol, ilty, ilwd, 
                   ngrps, nfacets, by_continuous, facet_by, type_info, 
                   ...) {
      grp_aes = type_info[["ul_col"]] == 1 || type_info[["ul_lty"]] == 
        ngrps || type_info[["ul_lwd"]] == ngrps
      if (length(x0) != 1) {
        if (!length(x0) %in% c(ngrps, nfacets, ngrps * 
                               nfacets)) {
        ...
        else if (!by_continuous && length(x0) == ngrps * 
                 nfacets) {
      ...
      arrows(x0 = x0, y0 = y0, x1 = x1, y1 = y1, length = length, angle = angle,
             code = code, col = arrow_col, lty = arrow_lty, lwd = arrow_lwd
