...
## Run cpt with a track_frame

check as.track_frame method for data.frame
```{r eval=FALSE, include=TRUE}
?as.track_frame.data.frame
```


Transform data to a track_frame. Trackframe always use x/y coordinates as cartesian coordinates with easting and northing. 
```{r}
...
## Run cpt with a track_frame and multiple IDs
...
Transform to track_frame
```{r}
move2_tf <- as.track_frame(albatross_move2)
class(move2_tf)
```
...
Transform to track_frame
```{r}
sftrack_tf <- as.track_frame(data)
class(sftrack_tf)
```
...
Transform to track_frame
```{r}
cocomo_tf <- as.track_frame_from_cocomo(xs, ys, timestamps, ids)
head(cocomo_tf)
class(cocomo_tf)
```
